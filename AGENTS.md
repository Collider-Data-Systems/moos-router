...
```
A Markdown file is never the final truth. HG is. **Live runtime truth: `ffs0/kb/superset/running-state.md` + `/healthz`.** Federation port map: `ffs0/dev/config/moos-federation.topology.json` (a human mirror, not read by the binary). Launcher: `D:\HPZ440\start_federation.ps1`.
...
```
- Default branch `master`. Runtime-code branches `feat/<purpose-slug>` (per `ffs0/AGENTS.md`); `feat/type-map-routing` (worktree `moos-router-feat-type-map-routing`) adds `--type-map` type-ID routing **not present on master**.
